...
- (void)targetFound:(NSNotification *)notification
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self->booksDelegate setLastScannedBook:aBook];
        
        [self->booksDelegate setOverlayLayer:self.targetOverlayView.layer];
...
- (void)bookWebDetailDismissed:(NSNotification *)notification
{
    ...
    //[self removeBookFromScreen];
    [self handleViewRotation:[[UIApplication sharedApplication] statusBarOrientation]];
}
...
{
    if ([[BooksManager sharedInstance] isNetworkOperationInProgress])
    {
