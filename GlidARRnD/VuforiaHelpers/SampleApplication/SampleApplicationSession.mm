...
// Get the AR rendering view area 
- (CGRect)getCurrentARViewBounds
{
    CGRect screenBounds = [[UIScreen mainScreen] bounds];
    CGRect viewFrame = screenBounds;
    
    // Scale the view bounds for the AR (OpenGL) view
...
- (void)initVuforiaInBackground
{
    ...
    @autoreleasepool {
        Vuforia::setInitParameters(mVuforiaInitFlags,"");
...
-(void) showCameraAccessWarning
{
    ...
    [UIAlertAction actionWithTitle:@"Close"
                           ...
                           handler:^(UIAlertAction *action) {
                               // Quit the app when user dismisses the camera access alert dialog
...
- (void) prepareAR  {
    ...
    CGSize viewBoundsSize = [self getCurrentARViewBounds].size;
...
- (bool) startAR:(Vuforia::CameraDevice::CAMERA_DIRECTION)camera error:(NSError **)error {
    CGSize ARViewBoundsSize = [self getCurrentARViewBounds].size;
